...
namespace SettingsApp
{
    public class BackupCodeLogic : MonoBehaviour
    {
        ...
        private string BuildBackupText()
        {
            ...
            sb.AppendLine($"BATTERY LEVEL  : {(SystemInfo.batteryLevel >= 0 ? (SystemInfo.batteryLevel * 100).ToString("F0", System.Globalization.CultureInfo.InvariantCulture) + "%" : "N/A")}");
            sb.AppendLine();
            sb.AppendLine("— HABIT —");
            ...
            for (int d = 30; d >= 0; d--)
            {
                ...
                {
                    case 28:
                        sb.AppendLine($"{date}  Sandra pinged: 'Prophecy ignored = prophecy fulfilled?'. Cassandra flag raised.");
                        break;
                    case 25:
                    ...
                    case 10:
                        ...
                        break;
                    case 7:
                        sb.AppendLine($"{date}  {gm.playerCigarettesPerDay} cigarettes logged. Paris texts: 'City still burning?'.");
                        break;
                    case 5:
                    ...
                    case 3:
                        ...
                        break;
                    case 0:
                        sb.AppendLine($"{date}  Backup reconstruction complete — if you're reading this, memory override failed.");
                        break;
                }
            }
            ...
            sb.AppendLine($"{today.AddDays(-35).ToString("yyyy-MM-dd")}  Sandra: \"hey {gm.playerName}, build uploaded. don't judge the placeholder art.\"");
            sb.AppendLine($"{today.AddDays(-35).ToString("yyyy-MM-dd")}  You   : \"placeholders are my jam.\"");
            sb.AppendLine($"{today.AddDays(-34).ToString("yyyy-MM-dd")}  Sandra: \"promise you'll actually play it this time?\"");
            sb.AppendLine($"{today.AddDays(-34).ToString("yyyy-MM-dd")}  You   : \"scout's honor.\"");
            sb.AppendLine($"{today.AddDays(-33).ToString("yyyy-MM-dd")}  Sandra: \"cool. it's short. just, be honest.\"");
            ...
            sb.AppendLine($"{today.AddDays(-31).ToString("yyyy-MM-dd")}  You   : \"still at the start. looking for secret code.\"");
            sb.AppendLine($"{today.AddDays(-31).ToString("yyyy-MM-dd")}  Sandra: \"there's no secret code. just choices.\"");
            ...
            sb.AppendLine($"{today.AddDays(-21).ToString("yyyy-MM-dd")}  You   : \"phone glitching, did you code that?\"");
            sb.AppendLine($"{today.AddDays(-21).ToString("yyyy-MM-dd")}  Sandra: \"i wish. that's on you {gm.playerName}.\"");
            sb.AppendLine($"{today.AddDays(-14).ToString("yyyy-MM-dd")}  Sandra: \"did you back up? please tell me you did.\"");
            sb.AppendLine($"{today.AddDays(-13).ToString("yyyy-MM-dd")}  You   : \"why?\"");
            sb.AppendLine($"{today.AddDays(-8).ToString("yyyy-MM-dd")}   You   : \"getting weird push alerts. says 'remember'. yours?\"");
            ...
            sb.AppendLine($"{today.AddDays(-11).ToString("yyyy-MM-dd")}  Paris : \"heard about the reset. bad idea {gm.playerName}.\"");
